...
public class AuthInterceptor implements HandlerInterceptor {
  ...
  @Value("${authentication.ignore-check}")
  ...
  private boolean isTokenBlacklisted(String token) {
    ...
    return redisTemplate.opsForValue().get(JWT + token) != null;
  }
  ...
  public boolean preHandle(
      ...
      throws Exception {
    if (ignoreCheck) {
      log.warn("Ignore authentication with this service. Please confirm!");
      return true;
    }
    ...
    String walletId = JwtUtils.getWalletIdFromJwtToken(token, rsaConfig.getPublicKey());
    httpServletRequest.setAttribute("username", username);
    httpServletRequest.setAttribute("walletId", walletId);
    return true;
  }
}
