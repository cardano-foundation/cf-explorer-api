...
public interface RedeemerRepository extends JpaRepository<Redeemer, Long> {
  ...
  @Query(
      "SELECT re.scriptHash AS scriptHash, txOut.address AS address, re.purpose as purpose,"
          ...
          + " LEFT JOIN StakeRegistration sdr ON sdr.txHash = tx.hash"
          + " LEFT JOIN StakeAddress sa ON (w.addr = sa OR de.address = sa OR sdr.address = sa.view)"
  ...
  @Query(value = "SELECT DISTINCT(r.tx.id) FROM Redeemer r " + " WHERE r.scriptHash = :scriptHash")
  List<Long> findTxIdsInteractWithContract(
