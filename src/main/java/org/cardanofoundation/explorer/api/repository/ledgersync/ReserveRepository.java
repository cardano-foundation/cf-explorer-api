...
public interface ReserveRepository extends JpaRepository<Reserve, Long> {
  ...
  @Query(
      "SELECT stake.view as stakeAddress, reserve.amount as amount"
          + " FROM Reserve reserve"
          + " INNER JOIN StakeAddress stake ON reserve.addr = stake"
          + " WHERE reserve.tx = :tx"
  ...
  @Query(
      "SELECT reserve.tx.id as txId, count(DISTINCT reserve.addr) as numberOfStakes, sum(reserve.amount) as rewards"
          + " FROM Reserve reserve"
