...
public interface EpochStakeRepository extends JpaRepository<EpochStake, Long> {
  ...
  @Query(
      value =
          "SELECT es.epochNo AS epochNo, sum(es.amount) AS totalStake FROM EpochStake es "
              ...
              + "GROUP BY es.epochNo")
  List<PoolActiveStakeProjection> totalStakeByEpochNoAndPool(
      @Param("epochNo") Set<Integer> epochNo, @Param("poolId") Long poolId);
  ...
  @Query(
      value =
          "SELECT sum(es.amount) FROM EpochStake es WHERE es.pool.id = :poolId "
  ...
  @Query(
      value =
          "SELECT es.epochNo as epochNo, pu.fixedCost as fee, sum(es.amount) as size "
              ...
              + "and es.epochNo between :epochBegin and :epochEnd "
              + "group by es.epochNo, pu.fixedCost")
  List<PoolReportProjection> getEpochSizeByPoolReport(
      @Param("poolView") String poolView,
      @Param("epochBegin") int epochBegin,
      @Param("epochEnd") int epochEnd);
  ...
  Page<PoolActiveStakeProjection> getDataForEpochList(
      @Param("poolId") Long poolId, Pageable pageable);
}
