...
public interface RewardRepository extends JpaRepository<Reward, Long> {
  ...
  @Query(
      value =
          "SELECT rw.earnedEpoch AS epochNo, sum(rw.amount) AS totalStake FROM Reward rw "
              + "WHERE rw.pool.id = :poolId AND rw.earnedEpoch IN :epochNo "
              + "GROUP BY rw.earnedEpoch")
  List<PoolActiveStakeProjection> totalRewardStakeByEpochNoAndPool(
      @Param("epochNo") Set<Long> epochNo, @Param("poolId") Long poolId);
  ...
  @Query(
      value =
          "SELECT ph.view AS view, sum(rw.amount) AS amount "
              + "FROM Reward rw "
              + "JOIN PoolHash ph ON rw.pool.id = ph.id "
              + "WHERE ph.view IN :poolViews AND rw.type = 'leader' AND rw.spendableEpoch = :epochNo "
              + "GROUP BY ph.view")
  List<PoolAmountProjection> getOperatorRewardByPoolList(
      @Param("poolViews") Set<String> poolViews, @Param("epochNo") Integer epochNo);

  @Query(
      value =
          "SELECT rw.pool.id AS poolId, sum(rw.amount) AS amount "
              + "FROM Reward rw "
              + "WHERE rw.pool.id IN :poolIds AND rw.type IN ('leader', 'member') AND rw.spendableEpoch = :epochNo "
              + "GROUP BY rw.pool.id")
  List<PoolAmountProjection> getPoolRewardByPoolList(
      @Param("poolIds") Set<Long> poolIds, @Param("epochNo") Integer epochNo);

  @Query(
      value =
          "SELECT rw.earnedEpoch AS epochNo, sum(rw.amount) AS amount FROM Reward rw "
              + "WHERE rw.pool.id = :poolId AND rw.type = 'member' "
              + "AND rw.earnedEpoch IN :epochNos "
              + "GROUP BY rw.earnedEpoch")
  List<EpochRewardProjection> getDelegatorRewardByPool(
      @Param("poolId") Long poolId, @Param("epochNos") Set<Integer> epochNos);
  ...
  @Query(
      value =
          "SELECT rw.earnedEpoch AS epochNo, sum(rw.amount) AS amount FROM Reward rw "
              ...
              + "AND rw.earnedEpoch IN :epochNos "
              + "GROUP BY rw.earnedEpoch")
  List<EpochRewardProjection> getPoolRewardByPool(
      @Param("poolId") Long poolId, @Param("epochNos") Set<Integer> epochNos);
