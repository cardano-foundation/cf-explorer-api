...
class AddressServiceTest {
  ...
  @Mock
  LatestAddressBalanceRepository latestAddressBalanceRepository;
  ...
  void getAddressDetail_shouldReturn() {
    ...
    when(addressRepository.getAddressDetail(addr)).thenReturn(
  ...
  void getAddressDetail_shouldAssociatedWithNativeScript() {
    ...
    when(addressRepository.getAddressDetail(addr)).thenReturn(
        AddressResponse.builder().address(addr).build());

    when(scriptRepository.findByHash(
        "4791b8567a68cab7332f158f766c536adf9cf70170079918de473826")).thenReturn(
        Optional.of(Script.builder().type(ScriptType.TIMELOCK).build()));
    AddressResponse response = addressService.getAddressDetail(addr);
    Assertions.assertTrue(response.isAssociatedNativeScript());
    Assertions.assertEquals(response.getScriptHash(),
                            "4791b8567a68cab7332f158f766c536adf9cf70170079918de473826");
  }
  ...
  void getAddressDetail_shouldAssociatedWithSmartContract() {
    ...
    when(scriptRepository.findByHash(
        "002059fc75b038a85b33ba4fdfaced4bed1247fbad24c538dd45245a")).thenReturn(
        Optional.of(Script.builder().type(ScriptType.PLUTUSV1).build()));
    AddressResponse response = addressService.getAddressDetail(addr);
  ...
  void getAddressAnalytics_shouldReturn() {
    ...
    when(addressTxAmountRepository.findMinMaxBalanceByAddress(any(), any(), any(),
                                                              any())).thenReturn(minMaxProjection);
  ...
  void getTopAddress_shouldReturn() {
    String addr = "addr1zy6ndumcmaesy7wj86k8jwup0vn5vewklc6jxlrrxr5tjqda8awvzhtzntme2azmkacmvtc4ggrudqxcmyl245nq5taq6yclrm";
    Pageable pageable = PageRequest.of(0, 10);
    LatestAddressBalance latestAddressBalance = LatestAddressBalance.builder().address(addr)
        .quantity(BigInteger.TEN).build();

    when(latestAddressBalanceRepository.findAll(pageable)).thenReturn(
        new PageImpl<>(List.of(latestAddressBalance)));

    when(addressTxCountRepository.findAllByAddressIn(List.of(addr))).thenReturn(
        List.of(AddressTxCount.builder().address(addr).txCount(1L).build()));
  ...
  void getTokenByDisplayName_shouldReturn() {
    ...
    TokenAddressResponse tokenAddressResponse = TokenAddressResponse.builder()
        .fingerprint("fingerprint").addressId(1L).address("address").policy("policy")
        .fingerprint("fingerprint").quantity(BigInteger.ONE)
        .displayName(HexUtils.fromHex("token", "fingerprint")).build();

    when(latestTokenBalanceRepository.findTokenAndBalanceByAddressAndNameView(any(), any(),
                                                                              any())).thenReturn(
        new PageImpl<>(addressTokenProjections));
  ...
  void getTokenByDisplayName_emptyDisplay() {
    ...
    when(latestTokenBalanceRepository.findTokenAndBalanceByAddress(addr, pageable)).thenReturn(
        new PageImpl<>(addressTokenProjections));
    when(tokenMapper.fromAddressTokenProjection(any(AddressTokenProjection.class))).thenReturn(
