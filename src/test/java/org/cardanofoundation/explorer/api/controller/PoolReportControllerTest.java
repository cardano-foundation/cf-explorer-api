...
public class PoolReportControllerTest {
  ...
  void testCreatePoolReport() throws Exception{
    UserPrincipal userPrincipal = UserPrincipal.builder()
        .username("username")
        .build();
    PoolReportCreateRequest bodyRequest = PoolReportCreateRequest.builder()
        .poolId("1")
        .isPoolSize(false)
    ...
    mockMvc.perform(post("/api/v1/pool-report/create")
        ...
        .content(asJsonString(Map.of("reportName","reportName","poolId","1","isPoolSize","false")))
        .with(request -> {
  ...
  void testListPoolReport() throws Exception{
    PoolReportListResponse poolReportListResponse = PoolReportListResponse.builder()
        .reportId(1L)
        .reportName("reportName")
        .isPoolSize(true)
        .build();
    ...
    verify(poolReportService).list(any(Pageable.class),any(),any(ReportHistoryFilterRequest.class));
  }
  @Test
  void testDetailEpochSizePoolReport() throws Exception {
    PoolReportDetailResponse.EpochSize epochSize = PoolReportDetailResponse.EpochSize.builder()
        .epoch("1")
        .size(BigDecimal.ONE)
        .fee(BigInteger.ONE)
        .build();
    ...
    when(poolReportService.fetchEpochSize(reportId,PageRequest.of(0,1),userPrincipal.getUsername()))
    ...
    verify(poolReportService).fetchEpochSize(reportId,PageRequest.of(0,1),userPrincipal.getUsername());
  }
  @Test
  void testExportExcelFile() throws Exception{
    ...
    when(poolReportService.export(anyLong(),any(ExportType.class),anyString())).thenReturn(response);

    MvcResult result = mockMvc.perform(get("/api/v1/pool-report/detail/{reportId}/export",reportId)
        .contentType(MediaType.APPLICATION_JSON)
  ...
  void testDetailPoolRegistration() throws Exception{
    ...
    when(poolReportService.fetchPoolRegistration(reportId,PageRequest.of(0,1),userPrincipal.getUsername()))
    ...
    verify(poolReportService).fetchPoolRegistration(reportId,PageRequest.of(0,1),userPrincipal.getUsername());
  }
  ...
  void testDetailPoolUpdate() throws Exception{
    ...
    when(poolReportService.fetchPoolUpdate(reportId,PageRequest.of(0,1),userPrincipal.getUsername()))
    ...
    verify(poolReportService).fetchPoolUpdate(reportId,PageRequest.of(0,1),userPrincipal.getUsername());
  }
  ...
  void testDetailRewardsDistribution()throws Exception {
    ...
    when(poolReportService.fetchRewardsDistribution(reportId,PageRequest.of(0,1),userPrincipal.getUsername()))
        .thenReturn(new BaseFilterResponse<>(List.of(response),1));
    ...
    verify(poolReportService).fetchRewardsDistribution(reportId,PageRequest.of(0,1),userPrincipal.getUsername());
  }
  ...
  void testDetailDeregistration() throws Exception{
    ...
    when(poolReportService.fetchDeregistraion(reportId,PageRequest.of(0,1),userPrincipal.getUsername()))
        .thenReturn(new BaseFilterResponse<>(List.of(response),1));
    ...
    verify(poolReportService).fetchDeregistraion(reportId,PageRequest.of(0,1),userPrincipal.getUsername());
  }
  ...
  void testDetailPoolReport() throws Exception{
    Long reportId = 1L;
    UserPrincipal userPrincipal = UserPrincipal.builder().username("username").build();
    PoolReportHistory response = PoolReportHistory.builder()
        .id(1L)
        .poolView("view")
        .isPoolSize(false)
        .build();

    when(poolReportService.detail(reportId,userPrincipal.getUsername()))
        .thenReturn(response);
