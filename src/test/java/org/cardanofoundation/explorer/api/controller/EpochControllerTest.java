...
public class EpochControllerTest {
  ...
  @MockBean private EpochService epochService;
  ...
  public void testGetEpochDetail() throws Exception {
    String epochNo = "1";
    EpochResponse expectedResponse =
    ...
    mockMvc
        .perform(get("/api/v1/epochs/{no}", epochNo))
  ...
  public void testGetBlockList() throws Exception {
    ...
    BaseFilterResponse<BlockFilterResponse> expectedResponse =
        new BaseFilterResponse<>(); // Set your expected response here
    ...
    mockMvc
        .perform(
            get("/api/v1/epochs/{no}/blocks", epochNo)
                .param("page", "1")
                .param("size", "20")
                .param("sort", "id,desc"))
        .andExpect(status().isOk())
  ...
  public void testFilter() throws Exception {
    ...
    BaseFilterResponse<EpochResponse> expectedResponse =
        new BaseFilterResponse<>(); // Set your expected response here
    ...
    mockMvc
        .perform(
            get("/api/v1/epochs").param("page", "1").param("size", "20").param("sort", "id,desc"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$").exists()) // Validate if data exists in the response
        // Add more assertions as needed based on your expected response
        .andReturn();
  ...
  public void testFindCurrentEpoch() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/epochs/current"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$").exists()) // Validate if data exists in the response
        // Add more assertions as needed based on your expected response
        .andReturn();
