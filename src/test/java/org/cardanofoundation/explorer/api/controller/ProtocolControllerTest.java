...
public class ProtocolControllerTest {
  ...
  @MockBean private ProtocolParamService protocolParamService;
  ...
  void testGetCurrentProtocolWithFilter() throws Exception {
    ...
    List<ProtocolType> protocolTypes =
        Arrays.asList(ProtocolType.MIN_FEE_A, ProtocolType.MIN_FEE_B);
    BigInteger startTime = BigInteger.valueOf(123456);
    BigInteger endTime = BigInteger.valueOf(789012);
    HistoriesProtocol mockHistoriesProtocol =
        new HistoriesProtocol(); // Create your mock response here
    ...
    mockMvc
        .perform(
            get(
                    "/api/v1/protocols/histories/filter/{protocolsTypes}",
                    Strings.collectionToCommaDelimitedString(
                        List.of(ProtocolType.MIN_FEE_A, ProtocolType.MIN_FEE_B)))
  ...
  void testGetLatestChange() throws Exception {
    ...
    // Perform the GET request and validate the response
    mockMvc
        .perform(get("/api/v1/protocols/latest"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$").exists());
  ...
  void testGetFixedProtocols() throws Exception {
    ...
    // Perform the GET request and validate the response
    mockMvc
        .perform(get("/api/v1/protocols/fixed"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$").exists());
