...
public class BlockControllerIntegrationTest {
  ...
  public void testGetBlockDetailByBlockId() throws Exception {
    ...
    BlockResponse mockResponse = BlockResponse.builder().hash(blockId).build();
    ...
    mockMvc
        .perform(get("/api/v1/blocks/{blockId}", blockId))
  ...
  public void testGetAll() throws Exception {
    ...
    List<BlockFilterResponse> mockResponse =
        Arrays.asList(new BlockFilterResponse(), new BlockFilterResponse());
    BaseFilterResponse<BlockFilterResponse> response =
  ...
  public void testGetTransactionsByBlock() throws Exception {
    ...
    // Perform the GET request
    mockMvc
        .perform(get("/api/v1/blocks/{blockId}/txs", blockId))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.data").isArray())
        .andExpect(jsonPath("$.data.length()").value(mockResponse.size()));
